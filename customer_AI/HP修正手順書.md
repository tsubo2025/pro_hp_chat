...
**プロテクトサービスページのスライドショー:**

1. 画像ファイルの配置場所: `public/images/protect/`
2. 現在の画像ファイル:
   - `slide_win10_support.png`
   - `slide_pc_repair.png`
   - `slide_ai_training.png`
3. 変更方法:
   - 既存画像の差し替え: 同名ファイルで上書き
   - 新規画像の追加: `src/config/slideshow.ts`の`PROTECT_SLIDES`配列を編集

**メインページのスライドショー:**

1. 画像ファイルの配置場所: `public/images/placeholder/`（または任意のフォルダ）
2. 現在の画像ファイル:
   - `slide-a.png`
   - `slide-b.png`
   - `slide-c.png`
3. 変更方法:
   - 既存画像の差し替え: 同名ファイルで上書き
   - 新規画像の追加: `src/config/slideshow.ts`の`MAIN_SLIDES`配列を編集

**設定ファイルの編集:**

- ファイル: `src/config/slideshow.ts`
- スライドショー画像のパスを管理
- 画像を追加・削除する場合は配列を編集
